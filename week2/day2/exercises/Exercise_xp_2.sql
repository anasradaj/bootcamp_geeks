...
-- SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Avengers';
...
-- WITH CheapestMovies AS (SELECT *,ROW_NUMBER() OVER (ORDER BY rental_rate ASC) as film_rank FROM film)
-- SELECT * FROM CheapestMovies WHERE film_rank > 10 AND film_rank <= 20;
